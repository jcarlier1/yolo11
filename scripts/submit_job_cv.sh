...
# Submit the cross-validation training job
JOB_ID=$(sbatch scripts/train_yolo_cv.slurm | grep -o '[0-9]*')
...
if [ ! -z "$JOB_ID" ]; then
    ...
    echo "  tail -f yolo11s_cv_training_${JOB_ID}.out"
    echo "  tail -f yolo11s_cv_training_${JOB_ID}.err"
    echo ""
