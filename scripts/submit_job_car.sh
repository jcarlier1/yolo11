...
DATASET_YAML=$(python -c "from src.yolo11.utils.config_utils import get_dataset_config; print(get_dataset_config('car')['data_yaml'])" 2>/dev/null | tail -n 1 | tr -d ' \n')
...
JOB_ID=$(sbatch scripts/train_yolo_car.slurm | grep -o '[0-9]*')
